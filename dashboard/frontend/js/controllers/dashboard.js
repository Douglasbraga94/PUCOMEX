...
.controller('DashboardCtrl', function($scope, $http) {
	...
	$scope.chamar = function () {
		...
		var url_aws = 'https://fq9rpuvvd9.execute-api.us-east-1.amazonaws.com/prod/pucomex-chamaURL';
		var perfil = $scope.input.perfil;

		var parser = document.createElement('a');
		parser.href = $scope.input.url;

		var url_pucomex = parser.protocol + '//' + parser.host + parser.pathname;
		var parametros = parser.search.substring(1).split('&');
		console.log(parametros)

		$http({
			method: 'POST',
			url: 'https://fq9rpuvvd9.execute-api.us-east-1.amazonaws.com/prod/pucomex-chamaURL',
			data: {
				url: url_pucomex,
